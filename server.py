...
def analyze_node(req: AnalysisRequest):
    ...
    if req.mode == "history":
        ...
        quiz_prompt = f"Create a {req.num_questions}-question multiple choice quiz about '{req.node}' at a {req.difficulty} difficulty level. Return ONLY valid JSON. The JSON should be an object with a key 'questions' which is a list of objects. Each question object must have: 'question' (string), 'options' (list of 4 strings), 'correct_index' (integer 0-3), and 'explanation' (string). Ensure the options are plausible distractors. Do not use markdown formatting."
